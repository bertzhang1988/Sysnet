...
public class Query {
	...
	public static String queryRoutePlan = "select PLA.PRIMARY_DOMICILE terminal, primaryTrm.terminal_alpha_cd  as terminal_Alpha"
			...
			+ " ,sum (pla.RAIL_LDD + pla.RAIL_ARR + pla.RAIL_ENR + pla.RAIL_LDG) as rail_Loads, sum (pla.PT_LDD + pla.PT_ARR + pla.PT_ENR + pla.PT_LDG) as pt_Loads"
			+ " ,sum (pla.ETME_BILLS)  as etme_Count, sum (pla.LATE_BILLS) as late_Count from slh_holistic_lane_vw pla, slh_terminal primaryTrm "
			+ "  where primary_domicile = terminal and regional_type_cd in (?)  group by  pla.primary_domicile, primaryTrm.terminal_alpha_cd,primaryTrm.terminal_nm  order by  pla.primary_domicile ";
